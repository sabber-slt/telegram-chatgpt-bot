...
function initEnv(env, i18n2) {
      ...
      switch (
        ...
        case "object":
          if (Array.isArray(ENV[key])) {
            ...
            ENV_VALUE_TYPE[key] &&
            Array.isArray(ENV_VALUE_TYPE[key])
          ) {
            ENV[key] = env[key].split(",");
          } else {
            try {
  ...
  }
  {
    if (
      ...
      if (
        env.BOT_NAME &&
        ENV.TELEGRAM_AVAILABLE_TOKENS.length === ENV.TELEGRAM_BOT_NAME.length
      ) {
  ...
  ENV.I18N = i18n2((ENV.LANGUAGE || "en").toLowerCase());
...
var Context = class {
  ...
  async initContext(message) {
    ...
    const replyId = CONST.GROUP_TYPES.includes(message.chat?.type)
      ? message.message_id
      : null;
...
async function sendPhotoToTelegram(url, token, context) {
  ...
  return await fetch(`${ENV.TELEGRAM_API_DOMAIN}/bot${token}/sendPhoto`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
async function sendChatActionToTelegram(action, token, chatId) {
  return await fetch(`${ENV.TELEGRAM_API_DOMAIN}/bot${token}/sendChatAction`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
async function getChatRole(id, groupAdminKey, chatId, token) {
  ...
  if (!groupAdmin || !Array.isArray(groupAdmin) || groupAdmin.length === 0) {
    ...
    await DATABASE.put(groupAdminKey, JSON.stringify(groupAdmin), {
...
async function gpt3TokensCounter() {
  const repo =
    "https://raw.githubusercontent.com/tbxark-archive/GPT-3-Encoder/master";
  const encoder = await resourceLoader(
    "encoder_raw_file",
    `${repo}/encoder.json`
  ).then((x) => JSON.parse(x));
...
async function requestCompletionsFromOpenAI(
  message,
  history,
  context,
  onStream
  ...
  if (
    onStream &&
    resp.ok &&
    resp.headers.get("content-type").indexOf("text/event-stream") !== -1
  ) {
...
async function requestCompletionsFromChatGPT(
  text,
  context,
  modifier,
  onStream
  ...
  if (!historyDisable) {
    originalHistory.push({
      role: "user",
      content: text || "",
      cosplay: context.SHARE_CONTEXT.role || "",
    });
    originalHistory.push({
      role: "assistant",
      content: answer,
    ...
    await DATABASE.put(historyKey, JSON.stringify(originalHistory)).catch(
      console.error
    );
  }
  return answer;
...
async function loadHistory(key, context) {
  ...
  if (context.SHARE_CONTEXT.role) {
    history = history.filter(
      (chat) => context.SHARE_CONTEXT.role === chat.cosplay
    );
  }
...
async function chatWithOpenAI(text, context, modifier) {
  try {
      ...
      onStream = async (text2) => {
        try {
          ...
          if (!context.CURRENT_CHAT_CONTEXT.message_id && resp.ok) {
            context.CURRENT_CHAT_CONTEXT.message_id = (
              await resp.json()
            ).result.message_id;
          }
        } catch (e) {
    ...
    const answer = await requestCompletionsFromChatGPT(
      text,
      context,
      modifier,
      onStream
    ...
    if (ENV.SHOW_REPLY_BUTTON && context.CURRENT_CHAT_CONTEXT.message_id) {
      try {
        await deleteMessageFromTelegramWithContext(context)(
          context.CURRENT_CHAT_CONTEXT.message_id
        );
...
async function commandUpdateRole(message, command, subcommand, context) {
  if (subcommand === "show") {
    ...
    if (size === 0) {
      return sendMessageToTelegramWithContext(context)(
        ENV.I18N.command.role.not_defined_any_role
      );
    }
          ...
          await DATABASE.put(
            ...
            JSON.stringify(
              Object.assign(context.USER_CONFIG, {
                USER_DEFINE: context.USER_DEFINE,
              })
            )
          );
          return sendMessageToTelegramWithContext(context)(
            ENV.I18N.command.role.delete_role_success
          );
        }
      } catch (e) {
        return sendMessageToTelegramWithContext(context)(
          ENV.I18N.command.role.delete_role_error(e)
        );
      }
    }
    return sendMessageToTelegramWithContext(context)(
  ...
  try {
    ...
    await DATABASE.put(
      context.SHARE_CONTEXT.configStoreKey,
      JSON.stringify(
        Object.assign(context.USER_CONFIG, { USER_DEFINE: context.USER_DEFINE })
      )
    );
    return sendMessageToTelegramWithContext(context)(
      ENV.I18N.command.role.update_role_success
    );
  } catch (e) {
    return sendMessageToTelegramWithContext(context)(
      ENV.I18N.command.role.update_role_error(e)
    );
  }
}
async function commandGenerateImg(message, command, subcommand, context) {
  ...
  try {
    setTimeout(
      () =>
        sendChatActionToTelegramWithContext(context)("upload_photo").catch(
          console.error
        ),
      0
    );
...
async function commandCreateNewChatContext(
  message,
  command,
  subcommand,
  context
  ...
  try {
    ...
    if (command === "/new") {
      return sendMessageToTelegramWithContext(context)(
        ENV.I18N.command.new.new_chat_start
      );
    } else {
      if (context.SHARE_CONTEXT.chatType === "private") {
        return sendMessageToTelegramWithContext(context)(
          ENV.I18N.command.new.new_chat_start_private(
            context.CURRENT_CHAT_CONTEXT.chat_id
          )
        );
      } else {
        return sendMessageToTelegramWithContext(context)(
          ENV.I18N.command.new.new_chat_start_group(
            context.CURRENT_CHAT_CONTEXT.chat_id
          )
        );
      }
...
async function commandUpdateUserConfig(message, command, subcommand, context) {
  ...
  try {
    ...
    return sendMessageToTelegramWithContext(context)(
      ENV.I18N.command.setenv.update_config_success
    );
  } catch (e) {
    return sendMessageToTelegramWithContext(context)(
      ENV.I18N.command.setenv.update_config_error(e)
    );
  }
}
...
async function commandFetchUpdate(message, command, subcommand, context) {
  ...
  if (!online) {
    online = await fetch(ts, config)
      .then((r) => r.text())
      .then((ts2) => ({ ts: Number(ts2.trim()), sha: "unknown" }))
...
async function commandUsage(message, command, subcommand, context) {
  if (!ENV.ENABLE_USAGE_STATISTICS) {
    return sendMessageToTelegramWithContext(context)(
      ENV.I18N.command.usage.usage_not_open
    );
  }
...
async function commandGenerateBill(message, command, subcommand, context) {
  ...
  return sendMessageToTelegramWithContext(context)(
    ENV.I18N.command.bill.bill_detail(
      bill.totalAmount,
      bill.totalUsage,
      bill.remaining
    )
  ...
  for (const key in commandHandlers) {
    if (message.text === key || message.text.startsWith(key + " ")) {
      ...
      try {
        if (command.needAuth) {
          ...
          if (roleList) {
            const chatRole = await getChatRoleWithContext(context)(
              context.SHARE_CONTEXT.speakerId
            );
            if (chatRole === null) {
              return sendMessageToTelegramWithContext(context)(
                ENV.I18N.command.permission.not_authorized
              );
            }
            if (!roleList.includes(chatRole)) {
              const msg = ENV.I18N.command.permission.not_enough_permission(
                roleList,
                chatRole
              );
              return sendMessageToTelegramWithContext(context)(msg);
        ...
        return sendMessageToTelegramWithContext(context)(
          ENV.I18N.command.permission.role_error(e)
        );
      }
      ...
      try {
        ...
        return sendMessageToTelegramWithContext(context)(
          ENV.I18N.command.permission.command_error(e)
        );
      }
    }
  }
...
async function msgIgnoreOldMessage(message, context) {
  if (ENV.SAFE_MODE) {
    ...
    try {
      idList =
        JSON.parse(
          await DATABASE.get(context.SHARE_CONTEXT.chatLastMessageIDKey).catch(
            () => "[]"
          )
        ) || [];
    } catch (e) {
    ...
    if (idList.includes(message.message_id)) {
      ...
      await DATABASE.put(
        context.SHARE_CONTEXT.chatLastMessageIDKey,
...
async function msgFilterWhiteList(message, context) {
  ...
  if (context.SHARE_CONTEXT.chatType === "private") {
    if (
      !ENV.CHAT_WHITE_LIST.includes(`${context.CURRENT_CHAT_CONTEXT.chat_id}`)
    ) {
      return sendMessageToTelegramWithContext(context)(
        ENV.I18N.message.user_has_no_permission_to_use_the_bot(
          context.CURRENT_CHAT_CONTEXT.chat_id
        )
      );
    }
  ...
  if (CONST.GROUP_TYPES.includes(context.SHARE_CONTEXT.chatType)) {
    ...
    if (
      !ENV.CHAT_GROUP_WHITE_LIST.includes(
        `${context.CURRENT_CHAT_CONTEXT.chat_id}`
      )
    ) {
      return sendMessageToTelegramWithContext(context)(
        ENV.I18N.message.group_has_no_permission_to_use_the_bot(
          context.CURRENT_CHAT_CONTEXT.chat_id
        )
      );
    }
...
async function msgFilterNonTextMessage(message, context) {
  if (!message.text) {
    return sendMessageToTelegramWithContext(context)(
      ENV.I18N.message.not_supported_chat_type_message
    );
  }
  return null;
}
...
async function msgProcessByChatType(message, context) {
  ...
  for (const handler of handlers) {
    try {
      ...
      console.error(e);
      return sendMessageToTelegramWithContext(context)(
        ENV.I18N.message.handle_chat_type_message_error(
          context.SHARE_CONTEXT.chatType
        )
      );
    }
  }
...
async function loadMessage(request, context) {
  ...
  if (ENV.DEV_MODE) {
    setTimeout(() => {
      DATABASE.put(
        `log:${/* @__PURE__ */ new Date().toISOString()}`,
        JSON.stringify(raw),
        { expirationTtl: 600 }
      ).catch(console.error);
    });
...
// src/router.js
var helpLink = "https://github.com";
var issueLink = "https://github.com/";
var initLink = "./init";
var footer = `
  `;
...
async function bindWebHookAction(request) {
  ...
  for (const token of ENV.TELEGRAM_AVAILABLE_TOKENS) {
    ...
    result[id] = {
      webhook: await bindTelegramWebHook(token, url).catch((e) =>
        errorToString(e)
      ),
      command: await bindCommandForTelegram(token).catch((e) =>
        errorToString(e)
      ),
    };
  }
  const HTML = renderHTML(`
      ...
      ${Object.keys(result)
        .map(
          ...
          <h4>Bot ID: ${id}</h4>
          <p style="color: ${
            result[id].webhook.ok ? "green" : "red"
          }">Webhook: ${JSON.stringify(result[id].webhook)}</p>
          <p style="color: ${
            result[id].command.ok ? "green" : "red"
          }">Command: ${JSON.stringify(result[id].command)}</p>
          `
        )
        .join("")}
        ${footer}
...
async function defaultIndexAction() {
  const HTML = renderHTML(`
      ...
      <br/>
      <p><strong><a href="${initLink}"> >>>>> کلیک کنید  <<<<< </a></strong> برای شروع ربات</p>
      <br/>
      ${ENV.API_KEY ? "" : buildKeyNotFoundHTML("API_KEY")}
      ${commandsDocument()
        .map(
          (item) =>
            `<p><strong>${item.command}</strong> - ${item.description}</p>`
        )
        .join("")}
      <br/>
...
async function loadBotInfo() {
  ...
  const HTML = renderHTML(`
      ...
      <p><strong>GROUP_CHAT_BOT_ENABLE:</strong> ${
        ENV.GROUP_CHAT_BOT_ENABLE
      }</p>
      <p><strong>GROUP_CHAT_BOT_SHARE_MODE:</strong> ${
        ENV.GROUP_CHAT_BOT_SHARE_MODE
      }</p>
      <p><strong>TELEGRAM_BOT_NAME:</strong> ${ENV.TELEGRAM_BOT_NAME.join(
        ","
      )}</p>
      ${Object.keys(result)
        .map(
          (id) => `
              ...
              <h4>Bot ID: ${id}</h4>
              <p style="color: ${
                result[id].ok ? "green" : "red"
              }">${JSON.stringify(result[id])}</p>
              `
        )
        .join("")}
      ${footer}
...
var fa_default = {
  ...
  utils: {
    not_supported_configuration: "این فرمت داده را قبول نمی کند",
  },
  message: {
    loading: "در حال بارگذاری",
    not_supported_chat_type: (type) =>
      `در حال حاضر این فرمت داده را پشتیبانی نمی کند.`,
    not_supported_chat_type_message:
      "در حال حاضر این فرمت داده را پشتیبانی نمی کند.",
    handle_chat_type_message_error: (type) => `سرور در حال حاضر پاسخ گو نیست`,
    user_has_no_permission_to_use_the_bot: (id) =>
      ` شما در حال حاضر اجازه دسترسی را ندارید، لطفا این آیدی را به ورکر خود اضافه کنید :(${id})`,
    group_has_no_permission_to_use_the_bot: (id) =>
      `The group has not enabled chat permissions, please contact the administrator to add the group ID (${id}) to the whitelist`,
  },
  command: {
    help: {
      summary: "این دستورها در حال حاضر پشتیبانی می شوند:\n",
      help: "راهنما",
      new: "شروع مکالمه جدید",
      start: "دریافت آی دی",
      img: "Gt moonlight`",
      version: "ساخت عکس با اجرای دستور /img",
      setenv:
        "Set user configuration, the complete command format is /setenv KEY=VALUE",
      delenv:
        "Delete user configuration, the complete command format is /delenv KEY",
      usage: "مقدار مصرف",
      system: "اطلاعات سیستم را مشاهده کنید",
      role: "هویت را بررسی کنید",
      redo: "بازگشت به مکالمه قبل",
      echo: "دیباگ",
      bill: "مشاهده مقدار مصرف",
    },
    role: {
      not_defined_any_role: "No roles have been defined yet",
      current_defined_role: (
        size
      ) => `The following roles are currently defined (${size}):
  `,
      ...
      update_role_error: (e) =>
        `Configuration item format error: \`${e.message}\``,
    },
    img: {
    ...
    new: {
      ...
      new_chat_start_private: (id) =>
        `A new conversation has started, your ID (${id})`,
      new_chat_start_group: (id) =>
        `A new conversation has started, group ID (${id})`,
    },
    setenv: {
      ...
      update_config_error: (e) =>
        `Configuration item format error: ${e.message}`,
    },
    version: {
      new_version_found: (current, online) =>
        `New version found, current version: ${JSON.stringify(
          current
        )}, latest version: ${JSON.stringify(online)}`,
      current_is_latest_version: (current) =>
        `Current is the latest version, current version: ${JSON.stringify(
          current
        )}`,
    },
    usage: {
  ...
  - Per chat usage: `,
    ...
    permission: {
      ...
      not_enough_permission: (roleList, chatRole) =>
        `Insufficient permissions, need ${roleList.join(
          ","
        )}, current: ${chatRole}`,
...
var en_default = {
  ...
  utils: {
    not_supported_configuration:
      "Not supported configuration or data type error",
  },
  message: {
    loading: "Loading",
    not_supported_chat_type: (type) =>
      `Currently not supported ${type} type of chat`,
    not_supported_chat_type_message:
      "Currently not supported non-text format messages",
    handle_chat_type_message_error: (type) =>
      `Error handling ${type} type of chat messages`,
    user_has_no_permission_to_use_the_bot: (id) =>
      `You do not have permission to use this bot, please contact the administrator to add your ID (${id}) to the whitelist`,
    group_has_no_permission_to_use_the_bot: (id) =>
      `The group has not enabled chat permissions, please contact the administrator to add the group ID (${id}) to the whitelist`,
  },
  command: {
